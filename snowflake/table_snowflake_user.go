...
func tableSnowflakeUser(_ context.Context) *plugin.Table {
	return &plugin.Table{
		...
		Columns: snowflakeColumns([]*plugin.Column{
			...
			{Name: "last_success_login", Type: proto.ColumnType_TIMESTAMP, Description: "Date and time when the user last logged in to the Snowflake."},
			{Name: "locked_until_time", Type: proto.ColumnType_TIMESTAMP, Description: "Specifies the number of minutes until the temporary lock on the user login is cleared."},
